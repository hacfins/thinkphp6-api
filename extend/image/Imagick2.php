...
class Imagick2
{
	...
	public function water($source, $locate = self::IMAGE_WATER_SOUTHEAST, $alpha = 80)
	{
		...
		if (empty($this->img))
		{
			E(\EC::PARAM_ERROR, '没有可以被添加水印的图像资源');
		}
		if (!is_file($source))
		{
			E(\EC::PARAM_ERROR, '水印图像不存在');
		}
	...
	public function text($text, $font, $size, $color = '#00000000', $locate = self::IMAGE_WATER_SOUTHEAST, $offset = 0, $angle = 0)
	{
		...
		if (empty($this->img))
		{
			E(\EC::PARAM_ERROR, '没有可以被写入文字的图像资源');
		}
		if (!is_file($font))
		{
			E(\EC::PARAM_ERROR, "不存在的字体文件：{$font}");
		}
