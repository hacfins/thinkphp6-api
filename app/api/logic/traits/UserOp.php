...
use app\api\model\
{
    log\UserLogs, rbac\UserTokens, report\ReportUser
};
use app\common\{
    facade\Os
};
use think\facade\
{Cache, Cookie, Request, Session};
...
trait UserOp
{
    ...
    public function Lg_Login(string $userName, string $token_id)
    {
        ...
        //统计信息
        $uR = ReportUser::instance();
        $rep_id = $uR->GetRepIdByDateNow();

        if(Os::isMobile())
        {
            $uR->IncLogins_Mobile($rep_id);
        }
        else
        {
            $uR->IncLogins_PC($rep_id);
        }
    }
    ...
    protected function Lg_AddUser(string $userName)
    {
        ...
        //统计信息
        $userStats = ReportUser::instance();
        $rep_id = $userStats->GetRepIdByDateNow();
        $userStats->IncUsers($rep_id);
    }
}